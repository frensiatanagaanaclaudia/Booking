...
module.exports = {
  ...
  deleteItem: async (req, res) => {
    try {
      //delete dri item id yg ada di kategori
      const { id } = req.params;
      ...
      function deleteImage() {
        for (let i = 0; i < item.image.length; i++) {
          Image.findOne({ _id: item.image[i]._id })
            .then((image) => {
              fs.unlink(path.join(`public/${image.imageUrll}`));
              image.remove();
            })
            .catch((error) => {
      ...
      async function deleteInfo() {
        ...
        Info.findOne({_id:item.info[i]._id}).then((info) => {
          fs.unlink(path.join(`public/${info.imageUrll}`));
          info.remove();
        }).catch((err)=> {
          res.status(500).json({ message: err.message });
        })
      ...
      async function deleteFeature() {
        for (let i = 0; i < item.feature.length; i++) {
          Feature.findOne({_id:item.feature[i]._id}).then((feature) => {
            fs.unlink(path.join(`public/${feature.imageUrll}`));
          feature.remove();
          }).catch((err)=> {
           res.status(500).json({message : err.message}) 
          })
